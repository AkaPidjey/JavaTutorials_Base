[Java Tutorials](README.md)

#PatternsAndAlgorithms

## Thema#1 Patterns
+ [Паттерн DAO](#Паттерн-DAO)
+ [RESTful сайты и сервисы](#RESTful-сайты-и-сервисы)


## Thema#2 Algorithms

+ [ССЫЛКИ НА ДОПОЛНИТЕЛЬНУЮ ИНФУ](#ССЫЛКИ-НА-ДОПОЛНИТЕЛЬНУЮ-ИНФУ)

[к оглавлению](#PatternsAndAlgorithms)

## Паттерн DAO
+ [Шаблон DAO в Java-->]( https://www.codeflow.site/ru/article/java-dao-pattern )
+ [Забудьте о DAO, используйте Repository-->]( https://habr.com/ru/post/263033/ )
+ [Паттерн Data Access Object-->]( http://javatutor.net/articles/j2ee-pattern-data-access-object )
+ [Java. Реализация шаблона DAO-->]( https://www.dokwork.ru/2014/02/daotalk.html )
+ [Использование паттерна data access object в клиентском приложении-->]( https://tproger.ru/projects/pattern-data-access-object/ )
+ [Урок Java 299: Patterns 32: DAO-->]( https://www.youtube.com/watch?v=cNHr5lvN-Hw )

[к оглавлению](#patterns)





## RESTful сайты и сервисы

### ---Info---
+ [Building REST services with Spring-->]( https://spring.io/guides/tutorials/rest/ )
### ---Video---
+ [Открытый урок «Создание REST-клиентов на Spring»(2019)-->]( https://habr.com/ru/company/otus/blog/440046/ )
+ [YouTube/OTUS/Разработка REST-клиентов на Spring // Бесплатный урок OTUS(2019)-->]( https://www.youtube.com/watch?v=2FN4378eG7E )
### ---Lessons---
+ [Обзор REST. Часть 1: что такое REST(2020)-->]( https://javarush.ru/groups/posts/2486-obzor-rest-chastjh-1-chto-takoe-rest )
+ [Обзор REST. Часть 2: коммуникация между клиентом и сервером(2020)-->]( https://javarush.ru/groups/posts/2487-obzor-rest-chastjh-2-kommunikacija-mezhdu-klientom-i-serverom- )
+ [Обзор REST. Часть 3: создание RESTful сервиса на Spring Boot(2020)-->]( https://javarush.ru/groups/posts/2488-obzor-rest-chastjh-3-sozdanie-restful-servisa-na-spring-boot )

+ [Spring MVC: создание веб-сайтов и RESTful сервисов(2020)-->]( https://habr.com/ru/post/500572/ )
### ---Forum---


+ [Название-->]( Ссылка )


[к оглавлению](#patterns)

## ССЫЛКИ НА ДОПОЛНИТЕЛЬНУЮ ИНФУ

### ---Info---
### ---Video---
### ---Lessons---
### ---Forum---
+ [Что такое паттерн проектирования MVC в Java?-->]( https://pro-java.ru/patterny-proektirovaniya-java/chto-takoe-pattern-proektirovaniya-mvc-v-java/ )
+ [Паттерны проектирования в Java [Часть 1]-->]( https://javarush.ru/groups/posts/496-patternih-proektirovanija-v-java )
+ [Паттерны проектирования в Java [Часть 2]-->]( https://javarush.ru/groups/posts/584-patternih-proektirovanija )
+ [Паттерны проектирования, используемые в Spring Framework-->]( https://habr.com/ru/company/otus/blog/451516/ )

### ---Forum---

+ [Название-->]( Ссылка )
+ [Название-->]( Ссылка )
+ [Название-->]( Ссылка )
+ [Название-->]( Ссылка )
+ [Название-->]( Ссылка )
+ [Название-->]( Ссылка )
+ [Название-->]( Ссылка )
+ [Название-->]( Ссылка )




[к оглавлению](#patterns)
